...
if (!isset($body['en'], $body['th']) || !is_array($body['en']) || !is_array($body['th'])) {
    http_response_code(422);
    echo json_encode(['error' => 'Payload must contain en and th objects']);
    exit;
}

$enKeys = array_keys($body['en']);
$thKeys = array_keys($body['th']);
$enSorted = $enKeys; $thSorted = $thKeys;
sort($enSorted); sort($thSorted);
if ($enSorted !== $thSorted) {
    http_response_code(422);
    echo json_encode(['error' => 'EN and TH key sets must match']);
    exit;
}
